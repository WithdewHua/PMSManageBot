...
async def check_expired_custom_lines():
    ...
    try:
        ...
        with get_session() as session:
            ...
            expiring_soon_lines = result.scalars().all()

            # 发送即将过期的提醒通知
            if expiring_soon_lines:
                logger.info(f"找到 {len(expiring_soon_lines)} 条即将过期的自定义线路")
                for line in expiring_soon_lines:
                    try:
                        # 计算剩余时间（小时）
                        remaining_hours = round(
                            (line.expires_at - current_time) / 3600, 1
                        )
            ...
            if not expired_lines:
                ...
                session.commit()
                logger.info(f"已将 {expired_count} 条过期自定义线路标记为已过期")
